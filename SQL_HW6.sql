...
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment06DB_AKoren')
	 Begin 
	  Alter Database [Assignment06DB_AKoren] set Single_user With Rollback Immediate;
	  Drop Database Assignment06DB_AKoren;
	 End
	Create Database Assignment06DB_AKoren;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment06DB_AKoren;
...
CREATE VIEW vInventoriesByEmployeesByDates AS
	SELECT DISTINCT TOP 10000 InventoryDate, EmployeeFirstName + ' ' + EmployeeLastName AS EmployeeName 
		FROM vInventories JOIN vEmployees ON vInventories.EmployeeID = vEmployees.EmployeeID
...
CREATE VIEW vEmployeesByManager AS
	SELECT TOP 100000 Mgr.EmployeeFirstName + ' ' + Mgr.EmployeeLastName AS Manager, Emp.EmployeeFirstName + ' ' + Emp.EmployeeLastName AS Employee
		FROM vEmployees AS Emp JOIN vEmployees AS Mgr ON Mgr.EmployeeID = Emp.ManagerID
...
CREATE VIEW vInventoriesByProductsByCategoriesByEmployees AS
	SELECT TOP 10000 vCategories.CategoryID, CategoryName, vProducts.ProductID, ProductName, UnitPrice, InventoryID, InventoryDate, [Count], Emp.EmployeeID, Emp.EmployeeFirstName + ' ' + Emp.EmployeeLastName AS Employee, Mgr.EmployeeFirstName + ' ' + Mgr.EmployeeLastName AS Manager
		FROM vCategories JOIN vProducts ON vCategories.CategoryID = vProducts.CategoryID 
			JOIN vInventories ON vInventories.ProductID = vProducts.ProductID 
				JOIN vEmployees AS Emp ON vInventories.EmployeeID = Emp.EmployeeID
					JOIN vEmployees AS Mgr ON Mgr.EmployeeID = Emp.ManagerID
						ORDER BY CategoryID, ProductID, ProductName;
GO
